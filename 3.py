...
class Window(wx.Frame):
    def check_log(self):
        ...
        if os.path.exists(path) == False:
            dlg = wx.MessageDialog(
                self, "Файл лога не найден. Файл будет создан автоматически", "Проверка лога", wx.OK)  # создаём всплывающее окно
            dlg.ShowModal()  # показываем окно
    ...
    def View(self, e):
        dlg = wx.MessageDialog(
            self, "Вы действительно хотите открыть лог? Данные последних поисков будут потеряны!", "Просмотр лога", wx.YES_NO)  # создаём всплывашку
